...
import RatingsBarChartEpisode from "./components/RatingsBarChartEpisode.jsx";
...
export default function Episodes() {
      ...
      .then((csv) => {
        Papa.parse(csv, {
          ...
          complete: (results) => {
            ...
            // Find episode from URL
            if (episodeId) {
              const foundEpisode = allEpisodes.find(
                (ep) => String(ep.episodeId) === String(episodeId),
              );

              console.log("Selected episode:", foundEpisode);
  ...
  const ratingsBreakdown = React.useMemo(() => {
    ...
    Object.keys(currentEpisode).forEach((key) => {
      const trimmedKey = key.trim();

      // só aceita "1" até "10"
      if (/^(10|[1-9])$/.test(trimmedKey)) {
        const value = Number(String(currentEpisode[key]).replace(/[^\d]/g, ""));
  ...
  // Load cover and trailer images dynamically
    useEffect(() => {
      const loadImages = async () => {
        ...
        if (currentEpisode?.Season && currentEpisode?.Number2 && currentEpisode?.Date3) {
          const episodeDate = new Date(currentEpisode.Date3.replace(/\u00A0/g, " "));
          const now = new Date();
  
          // 👉 só usa cover do episódio se JÁ TIVER IDO AO AR
          if (!isNaN(episodeDate) && now > episodeDate) {
            const seasonNum = `s${currentEpisode.Season}`;
            const episodeNum = currentEpisode.Episode2;
    ...
    <div>
      ...
      >
        ...
        >
          ...
          >
            ...
            >
              <a
                href={`/episodepage/${movieId}/${episodeId}`}
                style={{
                  display: "flex",
                  alignItems: "center",
                  gap: 1,
          ...
          >
            ...
            <div style={{ position: "relative", left: "3px" }}>
              ...
              >
                {currentEpisode ? currentEpisode.TitleName : firstRow.Title}
              </p>
              <p
                style={{
                  color: "white",
      ...
      <div style={{ display: "flex", margin: "0 auto", width: "1280px" }}>
        ...
        >
          ...
          >
            {ratingsBreakdown && (
              <RatingsBarChartEpisode ratings={ratingsBreakdown} />
            )}
          </div>
          <div
            style={{
          ...
          >
            ...
            >
              {currentEpisode.FinalRating} Unweighted mean
            </span>
          </div>
          <div>
            <img
